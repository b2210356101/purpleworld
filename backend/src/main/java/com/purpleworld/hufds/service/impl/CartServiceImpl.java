...
public class CartServiceImpl implements CartService {
    ...
    public AddToCartResponse addToCart(AddToCartRequest request,String email) {
        ...
        MenuItem menuItem = menuItemRepository.findById(request.getMenuItemId()).orElseThrow(() -> new RuntimeException("MenuItem not found"));
        ...
        CartGroup cartGroup = cart.getCartGroups().stream()
                .filter(cg -> cg.getRestaurant().getId().equals(restaurant.getId()))
                .findFirst()
                .orElseGet(() -> {
        ...
        // CartGroup içindeki CartItem'lara bak, bu ürün zaten varsa quantity artır

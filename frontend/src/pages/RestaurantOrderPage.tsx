  ...
  return (
    <Box sx={{ px: { xs: 2, sm: 4 }, py: 3 }}>
      ...
      {loading ? (
        ...
        <Box>
          {currentOrders.length > 0 ? (
            safeMap(currentOrders, (order, index) =>
                ...
                <Box sx={{ mb: { xs: 2, sm: 0 } }}>
                  <Typography variant="subtitle2" color="text.secondary">
                    {t("order.details.number", {
                      id: selectedOrder.orderGroupId,
                    })}
                  </Typography>
                  <Typography variant="body2" color="text.secondary">
                    {t("order.details.ordered")}:{" "}
                    {formatDate(selectedOrder.orderedDate)}
                  </Typography>
                  {selectedOrder.status === "REJECTED" && (
                    <Typography variant="body2" color="error">
                      {t("order.details.rejected")}:{" "}
                      {formatDate(selectedOrder.rejectionDate)}
                    </Typography>
                  )}
            ...
            >
              {selectedOrder.status === "ORDERED" && (
                <>
                  <Button
                    onClick={() =>
                      handleRejectOrder(selectedOrder.orderGroupId)
                    }
                    color="error"
                    variant="outlined"
                    startIcon={<CloseIcon />}
                  ...
                  <Button
                    onClick={() =>
                      handleAcceptOrder(selectedOrder.orderGroupId)
                    }
                    color="primary"
                    variant="contained"
                    startIcon={<CheckIcon />}
              ...
              {selectedOrder.status === "PREPARING" && (
                <>
                  <Button
                    onClick={() =>
                      handleMarkAsPrepared(selectedOrder.orderGroupId)
                    }
                    color="primary"
                    variant="contained"
                    startIcon={<CheckIcon />}
